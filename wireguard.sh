...
proto_wireguard_setup() {
  ...
  config_get peer_ip6      "${config}" "peer_ip6"
  ...
  for address in ${addresses}; do
    case "${address}" in
      ...
      *:*)
        if [ "${peer_ip6}" ]; then
	  EXTRA_CMD1="ip -6 addr add ${address} peer ${peer_ip6} dev ${config}"
        else
      ...
      *.*)
        if [ "${peer_ip}" ]; then
	  EXTRA_CMD0="ip addr add ${address} peer ${peer_ip} dev ${config} 2>&1"
	else
